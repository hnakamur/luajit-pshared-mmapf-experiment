...
local function close(fd, filename)
    if ffi.C.close(fd) == -1 then
        return errors.Errno:new(ffi.errno(), string.format("close filename=%s", filename))
    end
    return nil
end
...
function Mmap:open(filename, map_len)
    ...
    if fd == -1 then
        return nil, errors.Errno:new(ffi.errno(), string.format("open file=%s", filename))
    end

    local addr = ffi.cast("uint8_t *", ffi.C.mmap(nil, map_len, bit.bor(PROT_READ, PROT_WRITE), MAP_SHARED, fd, 0))
    if addr == nil then
        local err = errors.Errno:new(ffi.errno(), string.format("mmap file=%s", filename))
...
function Mmap:close()
    ...
    if ffi.C.munmap(self.addr, self.map_len) == -1 then
        err = errors.Errno:new(ffi.errno(), string.format("munmap file=%s", self.filename))
    end
